...
Examples include web-servers which must handle thousands of simultaneous content requests, 
  and petaflop supercomputers which routinely perform physical simulations that span tens of thousands of cpu cores. 
As improvements in clock frequency slow and hardware becomes increasingly parallel, 
  the performance of such systems becomes ever more a function of memory efficiency. 
However the ability to effectively reason about such behavior using existing tools such as valgrind or gdb, 
  through a text-based interface, 
  is limited and tedious at best.
